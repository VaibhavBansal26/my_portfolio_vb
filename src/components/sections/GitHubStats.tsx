...
export default function GitHubStats() {
  ...
  const statItems = [
    { label: "Repos",         value: fmt(data.publicRepos),       desc: "public" },
    { label: "Stars",         value: fmt(data.totalStars),         desc: "earned" },
    { label: "Languages",     value: `${data.languages}+`,         desc: "active" },
    { label: "Contributions", value: fmt(data.totalContributions), desc: "12 months" },
  ];
      ...
      <div style={{ maxWidth: 680, margin: "0 auto" }}>
        ...
        >
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", borderBottom: "1px solid var(--border)" }}>
            {statItems.map(({ label, value, desc }, i) => (
              <div key={label} style={{ padding: "14px 16px", borderRight: i < 3 ? "1px solid var(--border)" : undefined }}>
                <motion.div
                  initial={{ opacity: 0 }}
                  animate={loaded ? { opacity: 1 } : {}}
                  transition={{ duration: 0.4, delay: i * 0.08 }}
                  style={{ fontFamily: "'Syne',sans-serif", fontSize: 20, fontWeight: 800, color: "var(--accent)", lineHeight: 1 }}
                >
                  {value}
                </motion.div>
                <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 7, color: "var(--text-muted)", letterSpacing: ".15em", textTransform: "uppercase", marginTop: 3 }}>
                  {label}
                </div>
                <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 8, color: "var(--text-muted)", opacity: 0.5, marginTop: 1 }}>
                  {desc}
                </div>
              </div>
            ))}
